...
int test_connect_with_will_length() {
    IT("accepts a will and length");
    ShimClient shimClient;
    shimClient.setAllowConnect(true);

    byte connect[] = {0x10,0x2e,0x0,0x4,0x4d,0x51,0x54,0x54,0x4,0xe,0x0,0xf,0x0,0xc,0x63,0x6c,0x69,0x65,0x6e,0x74,0x5f,0x74,0x65,0x73,0x74,0x31,0x0,0x9,0x77,0x69,0x6c,0x6c,0x54,0x6f,0x70,0x69,0x63,0x0,0x9,0x01,0x02,0x03,0x04,0x00,0xff,0xfe,0xfd,0xfc};
    byte connack[] = { 0x20, 0x02, 0x00, 0x00 };
    shimClient.expect(connect,0x30);
    shimClient.respond(connack,4);

    PubSubClient client(server, 1883, callback, shimClient);
    uint8_t willMessage[] = {0x01,0x02,0x03,0x04,0x00,0xff,0xfe,0xfd,0xfc};
    int rc = client.connect((char*)"client_test1",(char*)"willTopic",1,0,willMessage,sizeof(willMessage));
    IS_TRUE(rc);
    IS_FALSE(shimClient.error());
...
int test_connect_with_will_length_username_password() {
    IT("accepts a will, length, username and password");
    ShimClient shimClient;
    shimClient.setAllowConnect(true);

    byte connect[] = {0x10,0x3e,0x0,0x4,0x4d,0x51,0x54,0x54,0x4,0xce,0x0,0xf,0x0,0xc,0x63,0x6c,0x69,0x65,0x6e,0x74,0x5f,0x74,0x65,0x73,0x74,0x31,0x0,0x9,0x77,0x69,0x6c,0x6c,0x54,0x6f,0x70,0x69,0x63,0x0,0x9,0x01,0x02,0x03,0x04,0x00,0xff,0xfe,0xfd,0xfc,0x0,0x4,0x75,0x73,0x65,0x72,0x0,0x8,0x70,0x61,0x73,0x73,0x77,0x6f,0x72,0x64};
    byte connack[] = { 0x20, 0x02, 0x00, 0x00 };
    shimClient.expect(connect,0x40);
    shimClient.respond(connack,4);

    PubSubClient client(server, 1883, callback, shimClient);
    uint8_t willMessage[] = {0x01,0x02,0x03,0x04,0x00,0xff,0xfe,0xfd,0xfc};
    int rc = client.connect((char*)"client_test1",(char*)"user",(char*)"password",(char*)"willTopic",1,0,willMessage,sizeof(willMessage),1);
    IS_TRUE(rc);
    IS_FALSE(shimClient.error());
...
int main()
{
    ...
    test_connect_with_will_length();
    test_connect_with_will_username_password();
    test_connect_with_will_length_username_password();
